...
class AdminDashboardController extends Controller
{
    ...
    public function dashboardAnalytics(Request $request): JsonResponse
    {
        ...
        // Total Businesses (active suppliers with a profile)
        $totalBusinesses = Supplier::where('status', 'active')
            ->whereHas('profile')
            ->count();
        ...
        // Paid Subscriptions (Premium users or those with active plan)
        $paidSubscriptions = Supplier::whereIn('plan', ['Premium', 'premium'])
            ->orWhereNotNull('subscription_plan_id')
            ->count();
        $previousPaidSubscriptions = Supplier::where(function($q) {
                $q->whereIn('plan', ['Premium', 'premium'])
                  ->orWhereNotNull('subscription_plan_id');
            })
            ->where('created_at', '<', $start)
            ->count();
        ...
        // Revenue - actual values from PaymentTransaction
        $revenue = (float) PaymentTransaction::where('status', 'completed')
            ->whereBetween('paid_at', [$start, $end])
            ->sum('amount');
        
        $previousRevenue = (float) PaymentTransaction::where('status', 'completed')
            ->whereBetween('paid_at', [$previousStart, $previousEnd])
            ->sum('amount');
            
        $revenueChange = $this->formatChange((int)$revenue, (int)$previousRevenue);
        ...
        $topCategories = SupplierProfile::select('business_categories')
            ...
            ->sortDesc()
            ->take(5)
            ->map(function ($count, $category) use ($start) {
                ...
                // Revenue per category (sum of payments from suppliers in this category)
                $categoryRevenue = PaymentTransaction::where('status', 'completed')
                    ->whereHas('user.profile', function($q) use ($category) {
                        $q->whereJsonContains('business_categories', $category);
                    })
                    ->where('paid_at', '>=', $start)
                    ->sum('amount');
                ...
                return [
                    ...
                    'revenue' => 'SAR ' . number_format($categoryRevenue),
                ];
            })
            ->values()
            ->all();

        // Revenue by Plan - Monthly vs Yearly only
        $revenueByPlan = [
            [
                'plan' => 'Monthly',
                'revenue' => (float) PaymentTransaction::where('status', 'completed')
                    ...
                    ->where('paid_at', '>=', $start)
                    ->sum('amount'),
                'users' => Supplier::whereHas('subscriptionPlan', function($q) {
                        $q->where('billing_cycle', 'monthly');
                    })->count(),
                'color' => 'bg-blue-500'
            ],
        ...
        $dailyUserActivity = $dateLabels->map(function (Carbon $date) use ($newSuppliersByDay, $inquiriesByDay) {
            ...
            // Get real revenue for this specific day
            $dailyRevenue = (float) DB::table('payment_transactions')
            ...
            return [
                ...
                'activeUsers' => (int) Supplier::where('status', 'active')
                    ->where(function($q) use ($dateString) {
                        $q->whereDate('last_seen_at', $dateString)
        ...
        // Detailed active subscriptions for the UI
        $activeSubscriptionsList = Supplier::whereIn('plan', ['Premium', 'premium'])
            ->orWhereNotNull('subscription_plan_id')
            ->with(['profile', 'subscriptionPlan'])
            ->get()
            ->map(function($supplier) {
                $lastTransaction = PaymentTransaction::where('supplier_id', $supplier->id)
                    ->where('status', 'completed')
                    ->latest('paid_at')
                    ->first();
                ...
                return [
                    ...
                    'business_name' => $supplier->profile->business_name ?? 'N/A',
                    'plan_name' => $supplier->subscriptionPlan->display_name ?? $supplier->plan,
                    'amount' => $lastTransaction ? (float)$lastTransaction->amount : 0,
                    'paid_at' => $lastTransaction ? $lastTransaction->paid_at->toDateTimeString() : 'N/A',
    ...
    public function analytics(Request $request): JsonResponse
    {
        ...
        // Revenue - actual data from PaymentTransaction
        $currentRevenue = (float) PaymentTransaction::where('status', 'completed')
            ->whereBetween('paid_at', [$start, $end])
            ->sum('amount');
            
        $prevRevenue = (float) PaymentTransaction::where('status', 'completed')
            ->whereBetween('paid_at', [$previousStart, $previousEnd])
            ->sum('amount');
        ...
        $analyticsData = [
            ...
            'revenue' => [
                ...
                'growth' => $this->percentageChange((int)$prevRevenue, (int)$currentRevenue),
                'data' => $this->fillMissingDays($dateLabels, $this->aggregateRevenueByDay($start, $end)),
            ],
        ];

        $revenueByPlan = SubscriptionPlan::all()->map(function ($plan) use ($start) {
            $users = Supplier::where('subscription_plan_id', $plan->id)->count();
            $planRevenue = PaymentTransaction::where('status', 'completed')
            ...
            return [
                ...
                'users' => $users,
                'color' => $this->planColor($plan->name),
            ];
        })->toArray();
    ...
    private function topCategories(): Collection
    {
        ...
        return $categories->map(function ($count, $category) {
            ...
            return [
                ...
                'revenue' => 'SAR ' . number_format($categoryRevenue),
                'growth' => 0, // Could be calculated if needed
            ];
        })->values();
    }

    private function aggregateRevenueByDay(Carbon $start, Carbon $end): Collection
    {
        return PaymentTransaction::where('status', 'completed')
            ->whereBetween('paid_at', [$start, $end])
            ->selectRaw('DATE(paid_at) as date_key, SUM(amount) as aggregate_sum')
            ->groupBy('date_key')
            ->pluck('aggregate_sum', 'date_key');
    }
